...
max <- max(season_data$viewers)
```

### Season ```r params$no_season``` summary

Season ```r params$no_season``` of Game of Thrones consisted of ```r no_episodes``` episodes that aired between ```r premiere``` and ```r end``` on HBO. The show gathered an average of ```r avg``` first-day TV viewers in the US, with the number growing from ```r min``` to ```r max``` million by the end of the season.
